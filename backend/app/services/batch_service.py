...
from app.policies.lifecycle import TransitionDecision, can_transition
...
def list_batches(db: Session, wave_id: str) -> list[Batch]:
    get_wave(db, wave_id)
    stmt = select(Batch).where(Batch.wave_id == wave_id).order_by(Batch.created_at)
...
def list_resources(db: Session, batch_id: str) -> list[Resource]:
    get_batch(db, batch_id)
    stmt = select(Resource).where(Resource.batch_id == batch_id).order_by(Resource.created_at)
    return list(db.scalars(stmt))


def advance_lifecycle(db: Session, batch_id: str, target_state: str) -> tuple[Batch, TransitionDecision]:
    batch = get_batch(db, batch_id)
    decision = can_transition(batch.lifecycle_state, target_state, batch.readiness_status)

    if decision.allowed:
        previous = batch.lifecycle_state
        batch.lifecycle_state = target_state
        db.commit()
        db.refresh(batch)
        audit_service.log_event(
            ...
            entity_id=batch.id,
            batch_id=batch.id,
            detail={"from": previous, "to": target_state},
        )
    else:
        audit_service.log_event(
            db,
            event_type="LIFECYCLE_TRANSITION_DENIED",
            entity_type="Batch",
            entity_id=batch.id,
            batch_id=batch.id,
            detail={"attempted": target_state, "reason": decision.reason},
        )
